...
<template>
	<div>{{ levelIndex+1 + " - " + level_names[levelIndex] }}</div>
	<EditorLayout @left-arrow="decrementLevel" @right-arrow="incrementLevel">
		<template #innerEditorContent>
			<img :src="level[levelIndex]" alt="The currently selected Level">
		</template>
	</EditorLayout>
</template>
...
export default defineComponent({
	...
	methods: {
		decrementLevel() {
			let levelData = readData("uint8", offsets.level_value) as number - 1;
			levelData = levelData < 1 ? 11 : levelData;
			writeData("uint8", offsets.level_value, levelData);
			this.updateLevel();
		},
		incrementLevel() {
			let levelData = readData("uint8", offsets.level_value) as number + 1;
		...
		updateLevel() {
			this.levelIndex = readData("uint8", offsets.level_value) as number - 1;
		}
	}
});
</script>
